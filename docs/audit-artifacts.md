...
- top-level JSON arrays, scalar lists, array-of-object access, element aliases and `foreach` aliases;
- bounded nested JSON array/object access, such as `$params[$i]["detail"][$j]["quantity"]`;
- limited unique local positional propagation from a route JSON variable into a local function parameter;
- `multipart/form-data` bodies proven by literal `$_POST[...]` and `$_FILES[...]` access;
